...
## CHARDEF blocks must use a number {#chardef_blocks_must_use_a_number}
...
`<spherescript>`{=html}\[CHARDEF 04000\] DEFNAME=c_scary_man ID=c_man
\...`</spherescript>`{=html}

You can just as easily do this:

`<spherescript>`{=html}\[CHARDEF c_scary_man\] ID=c_man // 0190 would
work here just as easily, since c_man = 0190
\...`</spherescript>`{=html}
...
`<spherescript>`{=html}\[FUNCTION examine_id\] // Usage: examine_id
`<id_to_examine>`{=html} // Example: examine_id c_man SYSMESSAGE \<EVAL
`<ARGS>`{=html}\>`</spherescript>`{=html}
...
## IF does not require an ENDIF {#if_does_not_require_an_endif}

All IF blocks require an ENDIF. At best, they won\'t work correctly. At
worst, they won\'t work at all. Let\'s look at an example:

`<spherescript>`{=html}\[FUNCTION superdude\] IF ( `<STR>`{=html} \< 100
) STR = \<EVAL `<STR>`{=html} + 2000\> IF ( `<DEX>`{=html} \< 100 ) DEX
= \<EVAL `<DEX>`{=html} + 2000\> IF ( `<INT>`{=html} \< 100 ) INT =
\<EVAL `<INT>`{=html} + 2000\>`</spherescript>`{=html}
...
`  STR = <EVAL ``<STR>`{=html}` + 2000>`\
`  IF ( ``<DEX>`{=html}` < 100 )`\
`     DEX = <EVAL ``<DEX>`{=html}` + 2000>`\
`     IF ( ``<INT>`{=html}` < 100 )`\
`        INT = <EVAL ``<INT>`{=html}` + 2000>`\
`     ENDIF`\
`  ENDIF`
...
ENDIF IF ( `<DEX>`{=html} \< 100 )

`  DEX = <EVAL ``<DEX>`{=html}` + 2000>`

ENDIF IF ( `<INT>`{=html} \< 100 )

`  INT = <EVAL ``<INT>`{=html}` + 2000>`

ENDIF`</spherescript>`{=html}

## SRC is required in functions {#src_is_required_in_functions}
...
`<spherescript>`{=html}\[FUNCTION fire_damage\] // The purpose of this
function is to cause damage // along with a spectacular flamestrike
effect SRC.EFFECT 3,i_fx_fire_column,1,16,0 SRC.DAMAGE \<EVAL {20
25}\>`</spherescript>`{=html}

So what\'s wrong with this? If we use it by saying SRC.FIRE_DAMAGE,
there is absolutely nothing wrong with it! However, what if we want to
use this function on someone other than the SRC? Let\'s say we make a
cursed shield that damages its bearer after a certain timer. Our \@Timer
event might look something like this:

`<spherescript>`{=html}ON=@Timer

`   CONT.FIRE_DAMAGE // Call the function on the CONT`\
`   RETURN 1 // Don't let the item decay``</spherescript>`{=html}
...
of the function, we\'re doing all of the damage and pretty effects to
the SRC object. We\'re in an \@Timer event. Can anyone tell me the
cardinal rule of \@Timer events? That\'s right. There is no SRC object.
And what if we were in another event which has a SRC, but that SRC
object was not the player we wanted to affect? As you can see, it\'s
much better to remove SRC from functions unless it\'s absolutely
necessary. Here\'s our new and improved function:

`<spherescript>`{=html}\[FUNCTION fire_damage\] // The purpose of this
function is to cause damage // along with a spectacular flamestrike
effect EFFECT 3,i_fx_fire_column,1,16,0 DAMAGE \<EVAL {20
25}\>`</spherescript>`{=html}

We\'ve removed all references to SRC from the function. Therefore, we
can call this using ANY object reference we want, including the example
...
can only think of one where it\'s absolutely necessary, and one where it
