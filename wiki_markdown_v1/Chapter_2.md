...
== `<font color="darkred">`{=html}\[RESOURCES\]`</font>`{=html} ==
...
`<font color="darkblue">`{=html}`[RESOURCES]`\
...
`<font color="darkred">`{=html}TYPE=t_gold`</font>`{=html}
...
each file, that\'s why we have this
`<font color="darkred">`{=html}scripts/`</font>`{=html} as the last file
on the \[RESOURCES\] tag, so sphere loads all other scripts that might
have been out of the list.*

== `<font color="darkred">`{=html}\[OBSCENE\]`</font>`{=html} ==

The next section is self-explanatory, \[OBSCENE\]. This defines
\"illegal names\". If a player creates a character with a name on this
list, it will tell him that the name is unavailable and to pick another.
If you are a punk who enjoys cussing at others, this list might be a
good place to expand your knowledge of obscenities.

== `<font color="darkred">`{=html}\[NOTOTITLES\]`</font>`{=html} ==

This defines the title that will be shown related to the player\'s
karma. Do not touch this section. If you remove a line inadvertently,
your server WILL NOT RUN. Only change them.

== `<font color="darkred">`{=html}\[PLEVEL X\]`</font>`{=html} ==
...
section as you can have various sections in different scripts, although
I recommend that you put them here just for organization purposes.*

== `<font color="darkred">`{=html}\[RUNES\]`</font>`{=html} ==
...
`//////// File Locations`\
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // Directory where spheretables.scp is located, from there we will`\
...
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // How often for Sphere to save your world (minutes)`\
`SavePeriod=20`\
\
...
`//////// Account Management`\
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` //Code for servers account application process`\
...
`//////// Client Management`\
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // What client protocol version used`\
...
`//////// Game Mechanics`\
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // Do not allow entering under roof being on horse?`\
`MountHeight=0`\
\
...
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // Distance in tiles before an NPC that's wandered too far from it's home will teleport back`\
`LostNPCTeleport=50`\
\
...
`///////////////////////////////////////////////////////////////`\
`</font>`{=html}` // Karma when player goes from good to neutral (from -10000 to 10000)`\
`PlayerNeutral=-2000`\
\
...
`// EF_Intrinsic_Locals = 00000020 //`**`Allows locals to be referenced differently. For example: <local.X> can be referenced as just ``<X>`{=html}**\
...
resource it gives to the player and the time for it to regen. Let\'s
take a look at this example (you\'ll see a commented text \"//t_tree\"
if you have read the previous chapter you\'ll surelly know what this is,
it\'s a defname)

`<font color="darkgreen">`{=html}`[REGIONRESOURCE mr_tree]``</font>`{=html}\
...
`<font color="darkred">`{=html}REAP=i_log`</font>`{=html}. This tells
the server to create an i_log item if this resource is taken. So you can
put anything you want to be gatherable in this REAP setting. The next
line, REGEN, defines how long this resource will take to reappear, in
seconds. As we can see here, it\'s 60\*60\*10 seconds, or 10 hours. As a
GM, if you walk around where players have been chopping wood or mining,
you\'ll see little worldgem bits all over the ground. They look like
spawnpoints, but are really resource markers. They mark where a specific
resource has been gathered, how much has been gathered, and what type of
resource it is. They also have a
`<font color="darkgreen">`{=html}TIMER`</font>`{=html} which counts down
from this very high number (36000) and then decay when it reaches zero.

`<font color="darkgreen">`{=html}`[REGIONTYPE r_default_rock t_rock]``</font>`{=html}\
`//Random rocks`\
...
`<font color="darkred">`{=html}`RESOURCES={ mr_iron 50 mr_copper 5 mr_gold 2 mr_rose 2 ...etc... }``</font>`{=html}
...
different format than you\'re used to. But it works exactly the same
way. It\'s also one of the ONLY places in SPHERE scripting that you can
use fractional numbers like 0.1 or 0.2. Actually you can\'t use them
here either, but SPHERE successfully hides this as long as you use them
in ALL of the
`<font color="darkgreen">`{=html}RESOURCES`</font>`{=html}. (By the way,
do not try to script a
`<font color="darkgreen">`{=html}RESOURCES`</font>`{=html} identifier as
a random selector. SPHERE wont like that. Use the format provided to
you.)

`<font color="darkgreen">`{=html}\[REGIONTYPE r_default_rock
t_rock\]`</font>`{=html} *Understanding:* That r_dafault_rock defines
the defname of that
`<font color="darkgreen">`{=html}REGIONTYPE`</font>`{=html}, which is
how you\'ll access it in other scripts (wait for next section =P). The
t_rock is the defname (see
`<font color="darkblue">`{=html}sphere_defs.scp`</font>`{=html} for it)
of the type of a rock, which shows sphere where your players can gather
that resource (you could change it to t_water so they could mine ores on
...
`<font color="darkgreen">`{=html}`[AREADEF a_world]``</font>`{=html}\
`NAME=Felucca`\
...
when doing .go \"region name\" (do not make 2 regions with the same
name, the last that has been loaded will replace the first when you do
.go command (which can fuck up a lot of things)).
...
Of course you\'re going to want to replace that \# with an actual
number! Where do we get those numbers? They\'re predefined in the game,
but they give us a nice handy list. I just showed it to you.
...
There you go. Your
`<font color="darkgreen">`{=html}FLAGS`</font>`{=html} would be set to
...
killed (because it\'s a safe region). Just take my word for it and
don\'t make a region safe and guarded.

Here\'s a quick list for the region flags (you should check you
sphere_defs.scp for more, cause they may be changed):

`<font color="darkblue">`{=html}`[DEFNAME region_flags]`\
`region_antimagic_all 00001 // ``<font color="darkred">`{=html}`all magic banned here.``</font>`{=html}\
`region_antimagic_recall_in 00002 // ``<font color="darkred">`{=html}`teleport,recall in to this, and mark``</font>`{=html}\
`region_antimagic_recall_out 00004 // ``<font color="darkred">`{=html}`can't recall out of here.``</font>`{=html}\
`region_antimagic_gate 00008 // ``<font color="darkred">`{=html}`can't open gates from or to this place.``</font>`{=html}\
`region_antimagic_teleport 00010 // ``<font color="darkred">`{=html}`can't teleport into here.``</font>`{=html}\
`region_antimagic_damage 00020 // ``<font color="darkred">`{=html}`just no bad magic here``</font>`{=html}\
`region_flag_ship 00040 // ``<font color="darkred">`{=html}`this is a ship region. ship commands``</font>`{=html}\
`region_flag_nobuilding 00080 // ``<font color="darkred">`{=html}`no building in this area``</font>`{=html}\
`region_flag_globalname 00100 // ``<font color="darkred">`{=html}`make sure the name is avail globally.``</font>`{=html}\
`region_flag_announce 00200 // ``<font color="darkred">`{=html}`announce to all who enter.``</font>`{=html}\
`region_flag_insta_logout 00400 // ``<font color="darkred">`{=html}`instant log out is allowed here. (hotel)``</font>`{=html}\
`region_flag_underground 00800 // ``<font color="darkred">`{=html}`dungeon type area. (no weather)``</font>`{=html}\
`region_flag_nodecay 01000 // ``<font color="darkred">`{=html}`things on the ground don't decay here.``</font>`{=html}\
`region_flag_safe 02000 // ``<font color="darkred">`{=html}`this region is safe from all harm.``</font>`{=html}\
`region_flag_guarded 04000 // ``<font color="darkred">`{=html}`try tag.guardowner``</font>`{=html}\
`region_flag_no_pvp 08000 // ``<font color="darkred">`{=html}`players cannot directly harm each other here.``</font>`{=html}\
`</font>`{=html}

And that\'s about it for the
`<font color="darkblue">`{=html}sphere_mapX.scp`</font>`{=html} files.
...
case, that is what\'s in this file. And books. Did I mention books?
Those impossible-to-write-correctly, rarely-used items that waste RAM
in-game?
...
various `<font color="darkgreen">`{=html}DEFNAMEs`</font>`{=html}. It\'s
these names that are important, not the contents, since you will almost
NEVER write your own list of names. I\'ve been using this emulator for
two years now, and I have never modified this file once.
...
`<font color="darkgreen">`{=html}**NAMES_HUMANMALE**`</font>`{=html}
list.

To make a list of names, it\'ll look like that:

`<font color="darkgreen">`{=html}`[NAMES A_DEFNAME_YOU_WANT]`\
`NUMBER_OF_NAMES`\
`NAMES`\
`</font>`{=html}
...
`<font color="darkgreen">`{=html}`[NAMES NAMES_SPECIAL]`\
`2`\
`Hil`\
`Hol`\
`</font>`{=html}
...
\"How do I give all blacksmiths 800 ingots?\" Now, I\'m not saying I
would like to play on a shard where you start with 10000 gold or
blacksmiths start with 800 ingots. But this file would be the place to
do it.

`<font color="darkblue">`{=html}`[NEWBIE MALE_DEFAULT]`\
...
template. It just uses the
`<font color="darkgreen">`{=html}**ITEMNEWBIE**`</font>`{=html} tag
rather than the
`<font color="darkgreen">`{=html}**ITEM**`</font>`{=html} tag. Look at
...
## sphere_skill.scp (SKILLCLASSES) {#sphere_skill.scp_skillclasses}
...
`<font color="darkblue">`{=html}`[SKILLCLASS 0]`\
`// undeclared class.`\
...
`<font color="darkgreen">`{=html}**EVENTS**`</font>`{=html}: It means
everyone with this skillclass will also call the events specified in
this field (to add more events just use a \",\", for example:
EVENTS=e_1,e_2,and so on)

`<font color="darkgreen">`{=html}**STATSUM**`</font>`{=html}: This is
how much the som of your players stats can be, for example, if I have
100 str, 100 dex and 100 int, I\'ve reached 300 stats, so I\'ve reached
STATSUM and can\'t have more (except for magic items and such things)

`<font color="darkgreen">`{=html}**SKILLSUM**`</font>`{=html}: The same
thing as STATSUM but for skills.

`<font color="darkgreen">`{=html}**STR/DEX/INT**`</font>`{=html}: This
is how much can the player have at this specific stat (except for magic
items and such)

`<font color="darkgreen">`{=html}**ALCHEMY**`</font>`{=html}: The same
thing as the STR, DEX and INT fields, but for skills (in this case
alchemy, but of course you can change to any other)

## sphere_serv_triggers.scp

This file hold special trigger \"functions\" that are fired by the
server its self at certain times depending on the function, below are a
list of the functions found in this file and a breif explain of what the
