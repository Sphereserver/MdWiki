...
`<spherescript>`{=html}ON=@SkillStart

`   IF (``<ACTION>`{=html}` == SKILL_HIDING)`\
...
## Random Skill Gain {#random_skill_gain}
...
## Modified Random Skill Gain {#modified_random_skill_gain}
...
`   IF (RAND(``<SWORDSMANSHIP>`{=html}`) < 10.0)`\
`       gain skill`\
`   ENDIF``</spherescript>`{=html}
...
## Linear Subskill Gain {#linear_subskill_gain}
...
## Skill Numbers as Variables {#skill_numbers_as_variables}
...
`<spherescript>`{=html}.01 100.0`</spherescript>`{=html}
...
`   IF (``<ARGN1>`{=html}` == Skill_Swordsmanship)`\
`       SWORDSMANSHIP += 0.1`\
`   ELIF (``<ARGN1>`{=html}` == Skill_hiding)`\
`       HIDING += 0.1`\
`   ELIF (``<ARGN1>`{=html}` == Skill_taming)`\
`       TAMING += 0.1`\
`   ELIF (``<ARGN1>`{=html}` == Skill_Macefighting)`\
`       MACEFIGHTING += 0.1`\
`   ELIF (``<ARGN1>`{=html}` == Skill_MAGICRESISTANCE)`\
`       MAGICRESISTANCE += 0.1`\
`   ELIF (``<ARGN1>`{=html}` == Skill_begging)`\
`       BEGGING += 0.1`\
`   ...`\
`   ...`\
`   ...``</spherescript>`{=html}\
...
`   TRY ``<ARGN1>`{=html}` = <EVAL <``<ARGN1>`{=html}`> + 0.1>``</spherescript>`{=html}\
`   `
