 Instead of setting the MORE1 property of a spawn to a single [character](Characters "wikilink") defname, a spawn group can be used instead so that a random character is spawned. A spawn group contains a set of character defnames, with optional weights to affect their chances of spawning.
...
```
 // // Lists the characters of a spawn in the console, and
```
picks one at random. // Usage: DescribeSpawn <spawn_defname> // \[FUNCTION describespawn\] // validate args IF !(\<`SERV.SPAWN.`<ARGS>\>)
...
```
\<`SERV.SPAWN.`<ARGS>.RESOURCES.COUNT\> characters) FOR 1 \<`SERV.SPAWN.`<ARGS>.RESOURCES.COUNT\>
...
```
\<`SERV.SPAWN.`<ARGS>.CALCMEMBERINDEX\> + 1\> `SERV.LOG` Picked '\<`SERV.SPAWN.`<ARGS>.RESOURCES.\<LOCAL.INDEX\>.KEY\>'. `RETURN`
