...
| [OWNER](OWNER "wikilink") | RW | Gets or sets the character that owns this character. Can be used to set an NPC's owner directly. |  |
...
| [ACTION](ACTION) | RW | Gets or sets the skill that the character is currently using. It returns the skill key. |  |
| [ACTP](ACTP) | RW | Gets or sets the character's ACTP value. It behaves like any other point, so you can extract x, y, z, and other point properties. |  |
...
| [BOUNCE](BOUNCE) *item_uid* | W | Places a specified item in the character's backpack. This will call the @DropOn_Self and @ItemDropOn_Self triggers on the character's backpack. |  |
...
| [FACE](FACE) *object_uid* or *coordinates* | W | Turns the character to face a specified object, SRC or coordinates. |  |
...
| [MESSAGE](MESSAGE) *message* | W | Displays a message above this character to SRC. If no hue is specified, it will use SpeechColor or SpeechColorOverride. |  |
...
| [SAY](SAY) *message* | W | Makes the character speak a message. If no hue is specified, it will use SpeechColor or SpeechColorOverride. |  |
...
| [SPEECHCOLOROVERRIDE](SPEECHCOLOROVERRIDE) *value* | RW | Overrides the client speech hue for players only. This will not affect Words of Power hue. Setting the value to 52 (or 0x34), which is the default yellow hue for player speech, disables the override. |  |
...
| [SPEECHCOLOR](SPEECHCOLOR "wikilink") | RW | For NPCs, gets or sets the colour of their speech. For players, this property is read-only and returns the last speech hue sent by the client. |  |
...
| [SYSMESSAGE](SYSMESSAGE "wikilink") *text* | W | Displays a system message to the client. If no hue is specified, it will use SpeechColor or SpeechColorOverride. |  |
