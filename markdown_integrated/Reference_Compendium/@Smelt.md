...
| ARGN2 | I | The total amount of different resources the item being smelted has.(For example a mace is made of iron ingots and logs, so the number of different resources is 2, for t_ore item the values is always 1. || local.resource.*n*.ID | IO | The ID of the itemdef that will be created if the smelting check is succesfull, n starts at 0. |
