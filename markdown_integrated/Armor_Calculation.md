...
### Body parts and Percents {#body_parts_and_percents}
...
### What layer covers what? {#what_layer_covers_what}
...
### Or, if you\'re preferring it the other way around: {#or_if_youre_preferring_it_the_other_way_around}
...
Do not forget that multiple coverings will _not_ stack; only the item
with the highest AR property will be taken into account!

### AR and AC Keywords {#ar_and_ac_keywords}
The keywords `AR` and `AC` (which is an alias for `AR`) can be used for items. They return the total protection provided by an armor piece. This value is calculated internally and is dependent on the item's `ARMOR`, `MODAR`, `ATTR_MAGIC` properties, and the `COMBAT_ELEMENTAL_ENGINE` option flag in `sphere.ini`. This calculation is primarily used when the `COMBAT_ELEMENTAL_ENGINE` is disabled.
