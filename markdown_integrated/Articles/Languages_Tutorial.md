...
retransmit what the player said (from the player's point of view nothing
happened). It is up to you to add what you want to the script and make
it work the way you want to.

In your *sphere.ini* look for the line:

<spherescript>SpeechSelf=spk_player // make sure it is equal to
spk_player</spherescript>

In sphere_speech.scp you'll find the **\[SPEECH spk_player\]** block.
Under it add the code as shown below:

<spherescript>\[SPEECH spk_player\] ON=\*
...
head, but allows npc's to react to things like "buy" and other commands.

Next, in a seperate file you can script the capture_speech function.
Here's the outline of what it looks like:

<spherescript>\[FUNCTION capture_speach\] REF1 = <UID>
